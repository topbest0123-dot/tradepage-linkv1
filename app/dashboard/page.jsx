...
export default function Dashboard() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data, error } = await supabase
        .from('profiles')
        .select(
          'slug,name,trade,city,phone,whatsapp,about,areas,services,prices,hours,facebook,instagram,tiktok,x,avatar_path,other_info' // <- added other_info
        )
        .eq('id', user.id)
        .maybeSingle();
      ...
      if (data) {
        setForm({
          ...
          tiktok: data.tiktok ?? '',
          x: data.x ?? '',
          other_info: data.other_info ?? '',   // <- added
        });
  ...
  const save = async () => {
    ...
    const row = {
      ...
      tiktok: form.tiktok,
      x: form.x,
      other_info: form.other_info,           // <- added
  ...
  return (
    <section>
      ...
      {/* NEW: Other Useful Information (optional) */}
      {textarea(
