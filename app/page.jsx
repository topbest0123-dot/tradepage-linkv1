...
export default function HomePage() {
  return (
    <main className="tp-home">
        ...
        <div className="container">
          <div className="hero-copy">
            ...
            <div className="chips" role="list">
              {['Tap-to-Call','Open WhatsApp','Quick Quote','Prices Up-Front','Services','Gallery','Social Proof','Share Anywhere'].map(t =>
      ...
      {/* VALUE BELT — modern marquee feel (scrollable on mobile) */}
      <section className="belt" aria-label="Highlights">
        <div className="belt-track">
          {['Essentials-Only','Ridiculously Fast','Mobile-First','Looks Professional','Shareable Anywhere','Frictionless','Simple Pricing','Easy to Edit']
            .map((t, i) => <span className="belt-pill" key={t+i}>{t}</span>)}
        </div>
      </section>

      {/* COMPARISON — not boxes, just two soft panels */}
      <section className="compare">
        <div className="container cmp-grid">
          <div className="cmp bad">
            <h3>Typical website</h3>
            <ul aria-label="Problems">
              {['Endless menus','Hidden contact','Slow + cluttered','Confusing copy','Too many steps'].map(s => <li key={s}>{s}</li>)}
            </ul>
          </div>
          <div className="cmp good">
            <h3>Your TradePage</h3>
            <ul aria-label="Benefits">
              {['Tap-to-call up-front','WhatsApp in one tap','Prices + services clear','Photos prove quality','Quote in seconds'].map(s => <li key={s}>{s}</li>)}
            </ul>
          </div>
        </div>
      </section>
...
/* Styles (uses your theme CSS vars) */
const styles = `
